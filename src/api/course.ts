...
export async function registerCourse(enrollmentId: string, courseId: string, session: string) {
    try {
        const { data, status } = await axiosInstance.post('/course/register', { enrollmentId, courseId, session });
        if (status == 200) {
            toast({
                title: "Success",
                description: data.message,
...
export async function getRegisteredCourses(departmentId: string) {
    try {
        const { data, status } = await axiosInstance.get('/course/registered/' + departmentId);
        if (status == 200) {
            return data.data
        } else {
            toast({
