import { getEnrollmentDetails } from "@/api/student"
...
export default function ProfileHome() {
    ...
    const { setCurrentDepartment, departmentList, refreshList, setEnrollmentDetails } = useDepartmentState()
    ...
    const onSelectDepartment = async (dept: Department) => {
        // fetch enrollment details
        const details = await getEnrollmentDetails(dept._id)
        console.log('enrollment', details)
        if (!details) {
            return toast({
                title: 'Confirm enrollment status',
                description: 'Failed to fetch enrollment details for this department',
                variant: 'warning'
            })
        }

        // set department and enrollment details
                ...
                <div className="w-full max-w-screen-xl mx-auto p-5 md:p-10 lg:p-20 grid gap-3 ">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-5">
                        ...
                        {departmentList.map(dept => (
                            <Card className="w-full max-w-sm " onClick={() => onSelectDepartment(dept)}>
                                <CardHeader >
